...
따라서 AI 계산에 필요한 매출/매입/공제 입력값은 별도 DB 조회가 아니라 AI 호출 payload로 전달합니다. (목데이터로 그냥 입력값 넣어버림 - DB에 추가해도 됩니다.)
...
AI 연동 시에는 버튼 클릭 시 AI 계산 API를 호출하고, 응답의 `tax_estimate` 값을 화면에 반영합니다. => 실시간 세액에 반영할 예정
